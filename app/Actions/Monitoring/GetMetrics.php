        ...
        return DB::table('metrics')
            ...
            ->select(
                [
                    ...
                    DB::raw('ROUND(AVG(cpu_usage), 2) as cpu_usage'),
                    DB::raw('ROUND(AVG(cpu_cores), 0) as cpu_cores'),
                    ...
                    DB::raw('ROUND(AVG(network_upstream), 2) as network_upstream'),
                    DB::raw('ROUND(AVG(network_downstream), 2) as network_downstream'),
                    DB::raw('ROUND(AVG(network_total_sent), 2) as network_total_sent'),
                    DB::raw('ROUND(AVG(network_total_received), 2) as network_total_received'),
                    DB::raw('ROUND(AVG(disk_read), 2) as disk_read'),
                    DB::raw('ROUND(AVG(disk_write), 2) as disk_write'),
                    DB::raw('ROUND(AVG(disk_tps), 2) as disk_tps'),
                    DB::raw('ROUND(AVG(io_wait), 2) as io_wait'),
                    $interval,
                ],
            )
